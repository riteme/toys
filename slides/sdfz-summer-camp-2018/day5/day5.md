...
Hasse 图中是不存在环的。假设存在环 $a \rightarrow b \rightarrow \cdots \rightarrow a$，根据 Hasse 图的定义，得到 $b \le a$；根据传递性，$a \le \cdots \le b \iff a \le b$；再根据反对称性，得到 $a = b$。同理可以推出整个环是一个点。所以 Hasse 图是拓扑图。
...
类似的，定义 $(i,\,A[i]) \ge (j,\,A[j]) \iff i \leqslant j \text{ and } A[i] \geqslant A[j]$。

---

令 $S = \{(k,\,A[k]): 1 \leqslant k \leqslant n\}$ 表示序列中所有元素，构造偏序集 $P = (S,\,\le)$，不难发现**每个不下降子序列对应偏序集中一条链**。
...
回到导弹拦截问题，一套系统能拦截的导弹是一个不上升子序列，也就是一条反链。现在要用最少的系统拦截所有导弹，相当于一个最小反链覆盖。根据 Mirsky 定理，最小反链覆盖的大小等于最长链的长度——最长不下降子序列的长度。
...
对于偏序集 $P_1 = (S,\,\le)$ 和偏序集 $P_2 = (S,\,\ge)$ 而言，这两个定理几乎是等价的，因为 $P_1$ 中的一条链就是 $P_2$ 中的一条反链，反之亦然。
...
接下来尝试证明我们可以构造出 $d$ 条链的覆盖。如果 $P$ 中没有比较关系（即其 Hasse 图中没有边），那么显然结论成立。此外当 $|S| = 0$ 时也成立。假设对于 $|S| = 0,\,1,\,\dots,\,n - 1$ 均成立，考虑当 $|S| = n$ 的情况：
...
对于 $S^+$ 的链覆盖中的 $d$ 条链，不难证明链的底端一定是 $\mathcal{A}$ 的元素，而 $M$ 处于其中某条链的顶端。对于 $S^-$ 也是类似的。因此可以用 $\mathcal{A}$ 中的元素作为中介，将 $S^-$ 和 $S^+$ 的两个链覆盖链接起来，得到一个大小为 $d$ 的链覆盖。<span style="float: right">$\blacksquare$</span>
...
每次合并果子时总堆数会减少 $k - 1$。还是尝试使用合并树这个模型，但是如果 $n$ 不是 $k - 1$ 的倍数，合并树上就可能出现空缺的叶子节点。

---

当然，可以加入一些没有重量的果堆，将 $n$ 补成 $k - 1$ 的倍数，这样既不影响答案，又可以使合并树上每个非叶子节点的儿子是满的。
...
这里也是一样的：考虑相邻两次合并操作：首先从堆中抽出 $a \leqslant b$，合并成 $a + b$ 后又放回堆。如果接下来的合并中又抽出了 $a  + b$，则这次合并出来的堆不会小于 $a + b$。否则将抽出 $b \leqslant c \leqslant d$，显然 $c + d$ 依然不小于 $a + b$。所以每次新合并的堆的重量是递增的。
